...
function generateToken(user) {
  return jwt.sign({ id: user.id, email: user.email, role: user.role || 'client' }, JWT_SECRET, { expiresIn: '7d' });
}
function authMiddleware(req, res, next) {
  ...
  try {
    ...
    var freshUser = db.prepare('SELECT id, email, role, plan, suspended, suspended_reason, bonus_messages, bonus_docs, full_name, company FROM users WHERE id = ?').get(decoded.id);
