...
// For ZIPs: zipProcessor gives us structured context + vision images; pdfAttach
// layers in the actual PDF content so Claude can see the drawings.
async function buildUserContentFromFiles(files) {
  ...
  for (const f of files || []) {
    ...
    if (ext === '.zip' && zipProcessor) {
      let zipTmpDir = null;
      try {
        // skipCleanup=true keeps the extracted PDFs on disk so pdfAttach can
        // read them off disk and attach them as document / image blocks.
        const zipData = await zipProcessor.processZip(f.path, uploadsDir, { skipCleanup: true });
        zipTmpDir = zipData.tmpDir;
        console.log(`[DeepRoutes] ZIP ${f.originalname} unpacked: total_files=${zipData.summary?.total_files}, pdf_count=${zipData.summary?.pdf_count}`);
        ...
        // (2) PDFs as document / rasterised image blocks via shared helper
        const pdfResult = attachZipPdfs(zipData, content);
        pdfNotes = pdfNotes.concat(pdfResult.pdfNotes);
        console.log(`[DeepRoutes] Attached ${pdfResult.attached} / ${pdfResult.pdfNotes.length} PDF(s)`);
