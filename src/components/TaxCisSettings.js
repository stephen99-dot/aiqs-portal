...
export default function TaxCisSettings() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={card}>
        <h3 style={h3}>Tax & CIS {saving && <span style={{ color: 'var(--text-muted)', textTransform: 'none' }}>· saving…</span>}{!saving && saved && <span style={{ color: '#10B981', textTransform: 'none' }}>· saved</span>}</h3>
        {error && <div style={{ color: '#EF4444', fontSize: 12.5, marginBottom: 10 }}>{error}</div>}

        <div style={rowGap}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            <input type="checkbox" checked={!!settings.vat_registered} onChange={e => save({ vat_registered: e.target.checked })} disabled={saving} style={{ width: 18, height: 18 }} />
        ...
        {!!settings.vat_registered && (
          <div style={rowGap}>
            ...
            <input style={inputStyle} defaultValue={settings.vat_number || ''} onBlur={e => save({ vat_number: e.target.value })} disabled={saving} placeholder="e.g. GB123456789" />
        ...
        <div style={rowGap}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            <input type="checkbox" checked={!!settings.cis_subcontractor} onChange={e => save({ cis_subcontractor: e.target.checked })} disabled={saving} style={{ width: 18, height: 18 }} />
        ...
        {!!settings.cis_subcontractor && (
          <div style={rowGap}>
            ...
            <select style={inputStyle} value={settings.cis_default_rate || 20} onChange={e => save({ cis_default_rate: Number(e.target.value) })} disabled={saving}>
        ...
        <div style={rowGap}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            <input type="checkbox" checked={!!settings.cis_contractor} onChange={e => save({ cis_contractor: e.target.checked })} disabled={saving} style={{ width: 18, height: 18 }} />
        ...
        <div style={{ marginBottom: 0 }}>
          ...
          <input style={inputStyle} defaultValue={settings.accountant_email || ''} onBlur={e => save({ accountant_email: e.target.value })} disabled={saving} placeholder="e.g. sam@yourbooks.co.uk" />
      ...
      <div style={card}>
        ...
        <div style={rowGap}>
          ...
          <select style={inputStyle} value={settings.card_fee_mode || 'absorb'} onChange={e => save({ card_fee_mode: e.target.value })} disabled={saving}>
