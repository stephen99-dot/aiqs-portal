    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {showResetModal && resetTarget && (
        <div className="modal-overlay" style={{ background: 'rgba(0,0,0,0.6)' }}>
          <div className="modal-card" style={{ background: t.card, border: '1px solid ' + t.border, padding: 28, width: 360, boxShadow: '0 20px 60px rgba(15,23,42,0.22)' }}>
      ...
      {showAdd && (
        <div style={{ background: t.card, border: '1px solid ' + (t.accent || '#F59E0B') + '40', borderRadius: 14, padding: '16px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 160px 60px 90px 90px', gap: 8, alignItems: 'end' }}>
            ...
            <div><label style={{ fontSize: 10, color: t.textMuted, display: 'block', marginBottom: 3 }}>Labour £</label><input style={{ ...iS, width: '100%' }} type="number" inputMode="decimal" step="0.01" placeholder="0.00" value={addData.labour_rate} onChange={e => setAddData(p => ({ ...p, labour_rate: e.target.value }))} /></div>
            <div><label style={{ fontSize: 10, color: t.textMuted, display: 'block', marginBottom: 3 }}>Material £</label><input style={{ ...iS, width: '100%' }} type="number" inputMode="decimal" step="0.01" placeholder="0.00" value={addData.material_rate} onChange={e => setAddData(p => ({ ...p, material_rate: e.target.value }))} /></div>
          </div>
                  ...
                  return (
                    <div key={rate.id} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 55px 80px 80px 90px 60px', minWidth: 520, padding: '9px 20px', alignItems: 'center', borderBottom: i < tradeRates.length - 1 ? '1px solid ' + t.border : 'none', background: isE ? (t.surfaceHover || t.surface) : 'transparent' }}>
                      {isE ? (<>
                        ...
                        <input style={{ ...iS, width: 70 }} type="number" inputMode="decimal" step="0.01" value={editData.labour_rate} onChange={e => setEditData(p => ({ ...p, labour_rate: e.target.value }))} />
                        <input style={{ ...iS, width: 70 }} type="number" inputMode="decimal" step="0.01" value={editData.material_rate} onChange={e => setEditData(p => ({ ...p, material_rate: e.target.value }))} />
